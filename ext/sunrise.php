	...
	class Sunrise7 {
		...
		function __construct( $args = array() ) {
			...
			$args = wp_parse_args( $args, array(
					...
					'css'         => 'assets/css',
					'js'          => 'assets/js',
					'views_class' => 'appfoliosync\Sunrise7_Views',
				) );
		...
		public function assets() {
			// Register styles
			wp_register_style(  'sunrise-' . $this->config['version'], $this->config['css'] . '/sunrise.css', false, $this->config['version'], 'all' );
			// Register scripts
			wp_register_script( 'sunrise-' . $this->config['version'], $this->config['js']. '/sunrise.js', array( 'jquery', 'jquery-form' ), $this->config['version'], true );
            // custom script
            wp_register_script( 'sunrise-custom-' . $this->config['version'], $this->config['js']. '/sunrise-custom.js', array('sunrise-' . $this->config['version']), $this->config['version'], true );
			// Add some l10n to JS
		...
		public function enqueue() {
			...
			// Enqueue scripts
			foreach ( array( 'jquery', 'jquery-form', 'farbtastic', 'sunrise-' . $this->config['version'] ,'sunrise-custom-' . $this->config['version']) as $script ) wp_enqueue_script( $script );
...
if ( !class_exists( 'Sunrise7_Views' ) ) {
	...
	class Sunrise7_Views {
        ...
        public static function type_url( $field, $config ) {
            $field = wp_parse_args( $field, array(
                'name'    => __( 'Url field', $config['textdomain'] ),
                'id'      => '',
                'desc'    => ''
            ) );
            return '<tr><th scope="row"><label for="sunrise-field-' . $field['id'] . '">' . $field['name'] . '</label></th><td><input type="url" value="' . get_option( $config['prefix'] . $field['id'] ) . '" name="sunrise[' . $field['id'] . ']" id="sunrise-field-' . $field['id'] . '" class="widefat" /><p class="description url">' . $field['desc'] . '</p></td></tr>';
        }

        public static function type_button( $field, $config ) {
            $field = wp_parse_args( $field, array(
                'name'    => __( 'Text field', $config['textdomain'] ),
                'id'      => '',
                'desc'    => ''
            ) );
            return '<tr><th scope="row"><label for="sunrise-field-' . $field['id'] . '">' . $field['name'] . '</label></th><td><input onclick="'.$field['callback'].';" type="button" value="' . $field['default'] . '" name="sunrise[' . $field['id'] . ']" id="sunrise-field-' . $field['id'] . '" class=" button button-primary" /><p class="button-field-description description ">' . $field['desc'] . '</p></td></tr>';
        }
